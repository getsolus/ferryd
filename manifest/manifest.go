...
func (t *Manifest) GetPaths() (ret []string) {
...
func NewManifest(path string) (mf *Manifest, err error) {
	...
	blob, err := ioutil.ReadFile(mf.Path)
	if err != nil {
		return
	}
	if _, err = toml.Decode(string(blob), mf); err != nil {
		return
	}
	mf.Head.Target = strings.TrimSpace(mf.Head.Target)
	mf.Head.Version = strings.TrimSpace(mf.Head.Version)
	if mf.Head.Version != "1.0" {
		err = ErrInvalidHeader
		return
	}
	if len(mf.Head.Target) == 0 {
	...
	for i := range mf.File {
		...
		if len(f.Path) == 0 || len(f.Sha256) == 0 {
			err = ErrInvalidPayload
			return
		}
		if !strings.HasSuffix(f.Path, ".eopkg") {
			err = ErrIllegalUpload
			return
		}
	}
	return
