...
func (p *Pool) Init(ctx *Context, d db.Database) error {
...
func (p *Pool) GetPoolItems(d db.Database) ([]*PoolEntry, error) {
	var ret []*PoolEntry
	err := d.Bucket([]byte(DatabaseBucketPool)).View(func(d db.ReadOnlyView) error {
		return d.ForEach(func(key, value []byte) error {
			var entry PoolEntry
			if err := d.Decode(value, &entry); err != nil {
				return err
			}
...
func (p *Pool) GetEntry(d db.Database, id string) (*PoolEntry, error) {
	bucket := d.Bucket([]byte(DatabaseBucketPool))
	entry := &PoolEntry{}
...
func (p *Pool) putEntry(d db.Database, entry *PoolEntry) error {
	return d.Bucket([]byte(DatabaseBucketPool)).PutObject([]byte(entry.Name), entry)
}

// GetSkipEntry will return the delta-skip entry for the given ID
func (p *Pool) GetSkipEntry(d db.Database, id string) (*DeltaSkipEntry, error) {
	bucket := d.Bucket([]byte(DatabaseBucketDeltaSkip))
...
// Private method to re-put the entry into the DB
func (p *Pool) putSkipEntry(d db.Database, entry *DeltaSkipEntry) error {
	return d.Bucket([]byte(DatabaseBucketDeltaSkip)).PutObject([]byte(entry.Name), entry)
}
...
func (p *Pool) AddDelta(d db.Database, pkg *libeopkg.Package, mapping *DeltaInformation, copyDisk bool) (*PoolEntry, error) {
	// Check if this is just a simple case of bumping the refcount
	if entry, err := p.GetEntry(d, pkg.ID); err == nil {
		entry.RefCount++
		return entry, p.putEntry(d, entry)
	}

	// Validate these source/target packages *actually* exist
	sourceEntry, err := p.GetEntry(d, mapping.FromID)
	if err != nil {
		return nil, err
	}
	targetEntry, err := p.GetEntry(d, mapping.ToID)
	if err != nil {
		return nil, err
	}
	...
	return p.addPackageInternal(d, pkg, copyDisk, mapping)
}

// addPackageInternal used by both AddDelta and AddPackage for the main bulk of
// the work
func (p *Pool) addPackageInternal(d db.Database, pkg *libeopkg.Package, copyDisk bool, delta *DeltaInformation) (*PoolEntry, error) {
	// Check if this is just a simple case of bumping the refcount
	if entry, err := p.GetEntry(d, pkg.ID); err == nil {
		entry.RefCount++
		return entry, p.putEntry(d, entry)
	...
	if err := p.putEntry(d, entry); err != nil {
...
func (p *Pool) AddPackage(d db.Database, pkg *libeopkg.Package, copy bool) (*PoolEntry, error) {
	return p.addPackageInternal(d, pkg, copy, nil)
}

// RefEntry will include the given eopkg if it doesn't yet exist, otherwise
// it will simply increase the ref count by 1.
func (p *Pool) RefEntry(d db.Database, id string) error {
	entry, err := p.GetEntry(d, id)
	if err != nil {
		return err
	}
	entry.RefCount++
	return p.putEntry(d, entry)
}
...
// storage.
func (p *Pool) UnrefEntry(d db.Database, id string) error {
	entry, err := p.GetEntry(d, id)
	if err != nil {
		return err
	}
	...
	// Now remove from DB
	b := d.Bucket([]byte(DatabaseBucketPool))
...
func (p *Pool) MarkDeltaFailed(d db.Database, id string, delta *DeltaInformation) error {
	// Already recorded? Skip again..
	if _, err := p.GetSkipEntry(d, id); err == nil {
		return nil
	}
...
func (p *Pool) GetDeltaFailed(d db.Database, id string) bool {
	skip, err := p.GetSkipEntry(d, id)
	if err == nil && skip != nil {
		return true
	}
	return false
