...
func (r *RepositoryManager) Init(ctx *Context, d db.Database) error {
...
func (r *RepositoryManager) GetRepos(d db.Database) ([]*Repository, error) {
	var ret []*Repository
	err := d.Bucket([]byte(DatabaseBucketRepo)).View(func(d db.ReadOnlyView) error {
		return d.ForEach(func(key, value []byte) error {
			var repo Repository
			if err := d.Decode(value, &repo); err != nil {
				return err
			}
			ret = append(ret, &repo)
			return nil
...
func (r *RepositoryManager) GetRepo(d db.Database, id string) (*Repository, error) {
	...
	var rTmp Repository
	rootBucket := d.Bucket([]byte(DatabaseBucketRepo))
...
// repo bucket.
func (r *RepositoryManager) CreateRepo(d db.Database, id string) (*Repository, error) {
	r.repoLock.Lock()
	defer r.repoLock.Unlock()

	if _, err := r.GetRepo(d, id); err == nil {
...
func (r *RepositoryManager) DeleteRepo(d db.Database, pool *Pool, id string) error {
	r.repoLock.Lock()
	defer r.repoLock.Unlock()

	repo, err := r.GetRepo(d, id)
	if err != nil {
	...
	// cycle
	err = d.Update(func(d db.Database) error {
		repoBucket := d.Bucket([]byte(DatabaseBucketRepo))
		...
		err := rootBucket.ForEach(func(k, v []byte) error {
			...
			for _, id := range entry.Available {
				if err := repo.removePackageInternal(d, pool, id); err != nil {
					return err
				}
			}
			...
			for _, id := range entry.Deltas {
				if err := repo.removeDeltaInternal(d, pool, id); err != nil {
					return err
				}
			}
...
func (r *Repository) GetEntry(d db.Database, id string) (*RepoEntry, error) {
	rootBucket := d.Bucket([]byte(DatabaseBucketRepo)).Bucket([]byte(r.ID)).Bucket([]byte(DatabaseBucketPackage))
	entry := &RepoEntry{}
...
func (r *Repository) putEntry(d db.Database, entry *RepoEntry) error {
	rootBucket := d.Bucket([]byte(DatabaseBucketRepo)).Bucket([]byte(r.ID)).Bucket([]byte(DatabaseBucketPackage))
...
func (r *Repository) RefDelta(d db.Database, pool *Pool, deltaID string) error {
	r.insertMut.Lock()
	defer r.insertMut.Unlock()

	// Ensure we REALLY have the delta.
	poolEntry, err := pool.GetEntry(d, deltaID)
	if err != nil {
		return err
	}

	// Now make sure we actually have the local entry
	entry, err := r.GetEntry(d, poolEntry.Meta.Name)
	if err != nil {
		return err
	}
	...
	if err := pool.RefEntry(d, deltaID); err != nil {
		return err
	}
	...
	return r.putEntry(d, entry)
}

// AddDelta will first open and read the .delta.eopkg, before passing it back off to AddLocalDelta
func (r *Repository) AddDelta(d db.Database, pool *Pool, filename string, mapping *DeltaInformation) error {
	...
	return r.AddLocalDelta(d, pool, pkg, mapping)
}
...
func (r *Repository) AddLocalDelta(d db.Database, pool *Pool, pkg *libeopkg.Package, mapping *DeltaInformation) error {
	r.insertMut.Lock()
	defer r.insertMut.Unlock()

	// Find our local package entry for the delta package first
	entry, err := r.GetEntry(d, pkg.Meta.Package.Name)
	if err != nil {
		return err
	}
	...
	// Grab the pool reference for this package
	if _, err = pool.AddDelta(d, pkg, mapping, false); err != nil {
		return err
	}
...
func (r *Repository) removePackageInternal(d db.Database, pool *Pool, id string) error {
	r.insertMut.Lock()
	defer r.insertMut.Unlock()

	poolEntry, err := pool.GetEntry(d, id)
	if err != nil {
		return nil
	}
	...
	return pool.UnrefEntry(d, id)
}

// removeDeltaInternal has the same job as removePackageInternal, but in future should
// be extended to remove the skip records
func (r *Repository) removeDeltaInternal(d db.Database, pool *Pool, id string) error {
	return r.removePackageInternal(d, pool, id)
}
...
func (r *Repository) UnrefPackage(d db.Database, pool *Pool, pkgID string) error {
	newHighest := 0
	var newHighestID string

	// Require a pool entry to remove it
	poolEntry, err := pool.GetEntry(d, pkgID)
	if err != nil {
		return err
	}
	...
	if err := r.removePackageInternal(d, pool, pkgID); err != nil {
		return err
	}
	...
	for _, deltaID := range entry.Deltas {
		pkgDelta, err := pool.GetEntry(d, deltaID)
		if err != nil {
			return err
		}
		...
		if pkgDelta.Delta.FromID == pkgID || pkgDelta.Delta.ToID == pkgID {
			if err := r.removeDeltaInternal(d, pool, pkgDelta.Name); err != nil {
				return err
			}
		} else {
	...
	for _, id := range entry.Available {
		...
		availEntry, err := pool.GetEntry(d, id)
		if err != nil {
			return err
		}
	...
	// Stuff it back into the d with the modified bits in place.
	return r.putEntry(d, entry)
}

// RefPackage will dupe a package from the pool into our own storage
func (r *Repository) RefPackage(d db.Database, pool *Pool, pkgID string) error {
	r.insertMut.Lock()
	defer r.insertMut.Unlock()
	...
	repoEntry := r.buildSaneEntry(d, pool, poolEntry.Meta, pkgID)
	// Already included
	if repoEntry == nil {
		return nil
	}
	...
	if err = pool.RefEntry(d, pkgID); err != nil {
		return err
	}
...
func (r *Repository) buildSaneEntry(d db.Database, pool *Pool, newPkg *libeopkg.MetaPackage, newID string) *RepoEntry {
	...
	entry, _ := r.GetEntry(d, newPkg.Name)
	...
	if entry != nil {
		...
		pkgAvail, err := pool.GetEntry(d, repoEntry.Published)
		if err == nil {
...
func (r *Repository) AddLocalPackage(d db.Database, pool *Pool, pkg *libeopkg.Package) error {
	r.insertMut.Lock()
	defer r.insertMut.Unlock()
	...
	repoEntry := r.buildSaneEntry(d, pool, &pkg.Meta.Package, pkg.ID)
	...
	// Grab the pool reference for this package (Always copy)
	if _, err := pool.AddPackage(d, pkg, false); err != nil {
		return err
	}
...
func (r *Repository) AddPackage(d db.Database, pool *Pool, filename string, anal bool) error {
	pkg, err := libeopkg.Open(filename)
	if err != nil {
		return err
	}
	...
	if !anal {
		return r.AddLocalPackage(d, pool, pkg)
	}

	// Do we have this?
	localPkg, err := r.GetEntry(d, pkg.Meta.Package.Name)
	if err != nil {
		return r.AddLocalPackage(d, pool, pkg)
	}

	// We have this package, so Published link must work
	published, err := pool.GetEntry(d, localPkg.Published)
	if err != nil {
		return err
	}
...
func (r *Repository) GetPackageNames(d db.Database) ([]string, error) {
	var pkgIds []string
	rootBucket := d.Bucket([]byte(DatabaseBucketRepo)).Bucket([]byte(r.ID)).Bucket([]byte(DatabaseBucketPackage))
...
func (r *Repository) GetPackages(d db.Database, pool *Pool, pkgName string) ([]*libeopkg.MetaPackage, error) {
	var pkgs []*libeopkg.MetaPackage

	entry, err := r.GetEntry(d, pkgName)
	if err != nil || entry == nil {
		return nil, err
	}

	for _, id := range entry.Available {
		p, err := pool.GetEntry(d, id)
		if err != nil {
			return nil, err
		}
...
func (r *Repository) CreateDelta(d db.Database, oldPkg, newPkg *libeopkg.MetaPackage) (string, error) {
...
func (r *Repository) HasDelta(d db.Database, pkgName, deltaPath string) (bool, error) {
	entry, err := r.GetEntry(d, pkgName)
	if err != nil {
		return false, err
	}
...
// ourselves
func (r *Repository) CloneFrom(d db.Database, pool *Pool, sourceRepo *Repository, fullClone bool) error {
	...
	rootBucket := d.Bucket([]byte(DatabaseBucketRepo)).Bucket([]byte(sourceRepo.ID)).Bucket([]byte(DatabaseBucketPackage))
	...
	for _, id := range copyIDs {
		if err := r.RefPackage(d, pool, id); err != nil {
			return err
		}
	}
	...
	for _, id := range deltaIDs {
		if err := r.RefDelta(d, pool, id); err != nil {
			return err
		}
	}
...
func (r *Repository) PullFrom(d db.Database, pool *Pool, sourceRepo *Repository) ([]string, error) {
	...
	err := rootBucket.ForEach(func(k, v []byte) error {
		...
		localEntry, _ := r.GetEntry(d, entry.Name)
		...
		tipVer, err := pool.GetEntry(d, entry.Published)
		if err != nil {
			return err
		}
		ourTip, err := pool.GetEntry(d, localEntry.Published)
		if err != nil {
			return err
		}
...
func (r *Repository) RemoveSource(d db.Database, pool *Pool, sourceID string, release int) error {
	...
	for _, id := range deleteIDs {
		if err = r.UnrefPackage(d, pool, id); err != nil {
			return err
		}
	}
...
func (r *Repository) CopySourceFrom(d db.Database, pool *Pool, sourceRepo *Repository, sourceID string, release int) error {
	var copyIDs []string
...
// with them.
func (r *Repository) TrimObsolete(d db.Database, pool *Pool) error {
	r.indexMut.Lock()
	...
	err := rootBucket.ForEach(func(k, v []byte) error {
		...
		for _, id := range entry.Available {
			...
			// Retain compatibility with eopkg, auto-drop -dginfo
...
func (r *Repository) TrimPackages(d db.Database, pool *Pool, maxKeep int) error {
